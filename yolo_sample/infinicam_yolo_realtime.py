...
from pypuclib import CameraFactory, GPUSetup
...
# カメラを開く（フレームレート・シャッター速度を 100 に固定）
cam = CameraFactory().create()
cam.setFramerateShutter(30, 30)
decoder = cam.decoder()
reso = cam.resolution()
W, H = reso.width, reso.height

# INFINICAMのGPUデコードはCUDA（NVIDIA GPU）専用。
# NVIDIA GPUが無いパソコンでは自動的にCPUデコードになる。
USE_GPU_DECODE = decoder.getAvailableGPUProcess()
if USE_GPU_DECODE:
    decoder.setupGPUDecode(GPUSetup(W, H))
print(f"デコード: {'GPU (CUDA)' if USE_GPU_DECODE else 'CPU'}")
...
def grab_frame():
    ...
    gray = (decoder.decodeGPU(cam.grab(), True, W) if USE_GPU_DECODE
            else decoder.decode(cam.grab()))
    if gray.ndim == 3:
        gray = gray[:, :, 0]
