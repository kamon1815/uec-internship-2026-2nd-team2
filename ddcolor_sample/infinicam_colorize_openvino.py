...
# グレー値→Labの L(uint8) 変換表。モノクロ入力なら誤差ゼロで
...
if use_gpu_decode:
    decoder.setupGPUDecode(GPUSetup(reso.width, reso.height))
    print("デコード: GPU")
else:
    print("デコード: CPU")
...
while True:
    ...
    xfer = cam.grab()
    gray = (decoder.decodeGPU(xfer, True, reso.width) if use_gpu_decode
            else decoder.decode(xfer))
    if gray.ndim == 3:                          # 念のため1chに揃える
        gray = gray[:, :, 0]
    h, w = gray.shape[:2]
